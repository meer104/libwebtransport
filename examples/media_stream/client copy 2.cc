...
int main() {
    ...
    client.onSessionOpen([](void* session_ptr) {
        ...
        void* stream_ptr = session->createBidirectionalStream();
        if (stream_ptr) {
            auto* stream = static_cast<web_transport::ClientStream*>(stream_ptr);
            ...
            stream->onStreamRead([depacketizer, output_file](std::vector<uint8_t> data) {
